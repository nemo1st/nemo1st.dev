...
async function build() {
  ...
  const facts = [["backend", "Rails / Python"], ["also", "PHP (Laravel)"], ["infra", "設計・構築・保守"], ["workflow", "GitHub 自動化"], ["community", "技術支援"], ["next gen", "コンテスト支援"]];
  const interview = [
    ["まず、何をしている人ですか？", "Webアプリケーション開発を中心に、インフラやデータ構造の解析まで幅広く手を動かすソフトウェアエンジニアです。「仕組みを知り、仕組みを最適化する」のが好きなWebエンジニアです。"],
    ["得意領域と技術スタックを教えてください。", `<dl class="stack-list"><div><dt>Backend</dt><dd>Ruby on Rails, Python, PHP (Laravel)</dd></div><div><dt>Infrastructure &amp; Ops</dt><dd>サーバー設計・構築、ドメイン管理、ランタイムやフレームワークのバージョンアップ・保守運用</dd></div><div><dt>Workflow &amp; AI</dt><dd>GitHub Issue / PR の自動化パイプライン、AIアシスタントを活用した開発効率化</dd></div></dl>`],
  ...
  const aboutBody = `<main id="main" class="about-page"><div class="about-layout"><div><header class="about-masthead"><img src="${sitePath("/assets/avatar.jpeg")}" alt="nemo1st" width="96" height="96"><div><p class="eyebrow">About</p><h1>nemo1st</h1><p class="role">Software Engineer</p><p class="lede">Ruby on Rails / Python / PHPで開発するソフトウェアエンジニア。Web開発から低レイヤー・データ構造解析、開発自動化まで「仕組みを深掘りしてハックする」のが好きです。快適な作業環境づくりと、コミュニティ向けのものづくり・技術支援にも力を入れています。</p></div></header><section class="interview">${sectionHeading("Interview")}${interview.map(([q, a], index) => `<article><span>${String(index + 1).padStart(2, "0")}</span><div><h2>${q}</h2><div class="answer">${a.startsWith("<") ? a : `<p>${a}</p>`}</div></div></article>`).join("")}<p class="transcript-end">— end of transcript</p></section></div><aside class="meta-rail"><section><h2>Profile</h2><dl>${facts.map(([key, value], index) => `<div><dt>${key}</dt><dd${index > 3 ? ' class="accent"' : ""}>${value}</dd></div>`).join("")}</dl></section><section><h2>Elsewhere</h2><a href="${GITHUB_URL}" target="_blank" rel="noopener noreferrer">${icon("github")}GitHub</a><a href="${sitePath("/rss.xml")}">${icon("rss")}RSS</a></section><section><h2>Navigate</h2><a href="${sitePath("/blog/")}">→ blog</a><a href="${sitePath("/blog/#shorts")}">→ shorts</a><a href="${sitePath("/talks/")}">→ talks</a></section></aside></div></main>`;
